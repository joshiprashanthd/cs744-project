...
int main(int argc, char **argv)
{
    if (argc != 3)
    {
        fprintf(stderr, "Usage: %s <messages_cnt> <topic_name>\n", argv[0]);
        exit(1);
    }
    ...
    while (i <= messages_cnt)
    {
        ...
        sendMessage(serviceId, topicName, message_id, "test message " + strMsgId);
        auto now = std::chrono::system_clock::now();
        std::time_t current_time = std::chrono::system_clock::to_time_t(now);
        std::cout << "[MessageId: " << message_id << "]" << " Message sent!" << std::endl;
        std::cout << "Message sent at: " << std::put_time(std::localtime(&current_time), "%Y-%m-%d %H:%M:%S") << std::endl;
        i++;
        sleep(2);
    }
