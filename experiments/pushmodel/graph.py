...
def compute_avg_latency(tsv_file_path):
    with open(tsv_file_path, "r") as tsv_file:
        ...
        for row in tsv_reader:
            ...
            end_time = datetime.datetime.strptime(row[1], "%Y-%m-%d %H:%M:%S.%f")
...
for file in os.listdir(output_folder_path):
    if file.startswith("subscriber"):
        ...
        avg_lat = compute_avg_latency(output_folder_path / file)
...
with open("stats.csv", "w") as csv_file:
    for num_req, avg_lat in temp:
        csv_file.write(f"{num_req},{avg_lat}\n")
