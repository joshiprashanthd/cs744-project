...
namespace core
{
  inline namespace v1
  {
    ...
    struct optional final
    {
      ...
      optional(optional const &that) : engaged{that.engaged}
      {
        if (not this->engaged)
        {
          return;
      ...
      optional(optional &&that) noexcept(std::is_nothrow_move_constructible<value_type>::value) : engaged{std::move(that.engaged)}
      {
        if (not this->engaged)
        {
          return;
      ...
      optional(value_type &&value) noexcept(
          std::is_nothrow_move_constructible<value_type>::value) : data{}, engaged{false}
      {
        allocator_type alloc{};
        std::allocator_traits<allocator_type>::construct(
            alloc,
      ...
      template <class... Args>
      explicit optional(in_place_t, Args &&...args) : data{},
                                                      engaged{false}
      {
        allocator_type alloc{};
        std::allocator_traits<allocator_type>::construct(
      ...
      value_type value_or(T &&val) const &
      {
        if (not this->engaged)
        {
          return value_type{std::forward<T>(val)};
        }
        return **this;
      }
      ...
      {
        if (not this->engaged and not that.engaged)
        {
          return;
        }
